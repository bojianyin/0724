...
<![endif]-->
	...
	<body class="user-select">
		...
		<button class="btn btn-default btn-search" name="search" type="submit">搜索</button>
						...
						<?php
 $cate=M("category")->order("sort")->select(); import("Class.Category",APP_PATH); $cate=Category::arrayMergeChilds($cate); ?>
						
						<ul class="nav navbar-nav navbar-right">
							<?php if(is_array($cate)): foreach($cate as $key=>$v): ?><li>
									<a data-cont="" title="<?php echo ($v["name"]); ?>" href=""><?php echo ($v["name"]); ?></a>
									<?php if($v['child']): ?><ul>
										<?php if(is_array($v["child"])): foreach($v["child"] as $key=>$value): ?><li><a data-cont="" title="<?php echo ($value['name']); ?>" href=""><?php echo ($value['name']); ?></a></li><?php endforeach; endif; ?>
									</ul><?php endif; ?>
