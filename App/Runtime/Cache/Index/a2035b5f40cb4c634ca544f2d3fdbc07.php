	...
	<body class="user-select">
		...
		<button class="btn btn-default btn-search" name="search" type="submit">搜索</button>
						...
						<ul class="nav navbar-nav navbar-right">
								<?php
 $_nav_cate=M('category')->order('sort asc')->select(); import('Class.Category',APP_PATH); $_nav_cate=Category::arrayMergeChilds($_nav_cate); foreach ($_nav_cate as $v) : extract($v); ?>			<li>
									<a data-cont="" title="" href=""><?php echo ($name); ?></a>
									<?php if($child): ?><ul>
										<?php if(is_array($child)): foreach($child as $key=>$value): ?><li><a data-cont="" title="" href=""><?php echo ($value['name']); ?></a></li><?php endforeach; endif; ?>
									</ul><?php endif; ?>
								</li><?php endforeach ?>
						</ul>
		...
		<section class="container">
			<div class="content-wrap">
				<div class="content">
					...
					<?php if(is_array($list)): foreach($list as $key=>$v): ?><article class="excerpt excerpt-1" style="">
						...
						<header>
							...
							<h2><a href="#" title="" target="_blank" ><?php echo ($v["name"]); ?></a>
			</h2>
						</header>
						<?php if(is_array($v["blog"])): foreach($v["blog"] as $key=>$value): ?><p class="note"><?php echo ($value["content"]); ?></p><time class="time"><i class="glyphicon glyphicon-time"></i><?php echo date('m/d',$value['time']);?></time><?php endforeach; endif; ?>
